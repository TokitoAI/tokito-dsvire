...
def run(
    ...
    *,
    ...
    documents = tuple(item for item in visual.documents if item.split == split)
    selected_queries = tuple(item for item in queries.queries if item.split == split)
    ...
    runtime = {
        "device": device,
        "device_map": encoder.hf_device_map,
        "load_plan": {
            "requested": encoder.load_plan["requested"],
            "device_map": encoder.load_plan["device_map"],
            "max_memory": encoder.load_plan["max_memory"],
        },
