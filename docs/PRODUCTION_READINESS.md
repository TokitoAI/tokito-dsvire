...
| DSV-006 | P1 | Partial: [#342](https://github.com/TokitoAI/tokito/issues/342) | The current branch adds a strict provenance registry, group-level split leakage checks, download-only policy, hash verification, deterministic JSON metrics, a three-document development gate, and release artifact publication. The 500-document/2,000-query corpus, held-out groups, visual labels, and legal review are still missing. | Reproducible provenance registry; dev/test splits; legal review; baseline runners; published metric artifacts. |
