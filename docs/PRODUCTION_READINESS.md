...
|---|---|---|
| `python -m pytest -q` on Windows/Python 3.11 | 111 passed, 1 skipped on the verifier-provenance branch | Unit/fixture/API/evaluator tests; not corpus accuracy or load evidence |
...
| DSV-003 | P0 | Partial on [#345](https://github.com/TokitoAI/tokito/issues/345) | Evidence v2 removes ambiguous `verified`/`verify_confidence`, names the heuristic method/policy/outcome, and prevents heuristic scores from claiming calibrated-probability semantics. Actual EGVV and held-out calibration remain absent. | Calibrated held-out results, adversarial near-miss set, and wrong-figure rate at or below the SLO. |
...
| DSV-013 | P1 | Partial on [#345](https://github.com/TokitoAI/tokito/issues/345) | Evidence v2 now requires accepted pinout, table, and package regions plus identity grounding provenance. Producer and canonical Rust contract are implemented; downstream consumer migration is still in progress. | Versioned contract decision implemented by every producer/consumer with cross-repository contract tests. |
