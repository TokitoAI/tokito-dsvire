...
| Frozen dependency graph | Universal `uv.lock` SHA-256 `5524bc1a...24af0` covers runtime/test/visual/OpenCLIP; runtime export SHA-256 `75add53d...0e4a` contains exact versions and artifact hashes; local release verifier passes 182 tests, strict typing across 25 package/script files, lint/format, no-isolation build, and hash-pinned runtime audit | Docker boundary/build evidence must come from Linux CI because Docker is unavailable on this workstation; byte-identical image output has not yet been demonstrated |
